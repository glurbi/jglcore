...
/**
 * This first example aims at providing the bare bone code for drawing a white triangle.
 */
public class Tutorial01 implements GLEventListener {
    ...
    private void createTriangle(GL3 gl3) {
        ...
        gl3.glBufferData(GL3.GL_ARRAY_BUFFER, 4*3*3, buf, GL3.GL_STATIC_DRAW);
        triangleId = buffers[0];
    }
    
    private void renderTriangle(GL3 gl3) {
        gl3.glClear(GL3.GL_COLOR_BUFFER_BIT);
        gl3.glEnableVertexAttribArray(POSITION_ATTRIBUTE);
        gl3.glBindBuffer(GL3.GL_ARRAY_BUFFER, triangleId);
        gl3.glVertexAttribPointer(POSITION_ATTRIBUTE, 3, GL3.GL_FLOAT, false, 0, 0);
        gl3.glDrawArrays(GL3.GL_TRIANGLES, 0, 3);
        gl3.glDisableVertexAttribArray(POSITION_ATTRIBUTE);
    }
    ...
    private void createSimpleProgram(GL3 gl3) {
        ...
        gl3.glBindAttribLocation(programId, POSITION_ATTRIBUTE, "inPosition");
        gl3.glLinkProgram(programId);
        checkProgramStatus(gl3, programId);
    }
    ...
    private void checkProgramStatus(GL3 gl3, int programId) {
        int[] params = new int[1];
